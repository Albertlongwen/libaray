...
#   if ELPP_OS_UNIX
#      if ELPP_OS_ANDROID
    static const char* kDefaultLogFile                         =      "/sdcard/minetang/myeasylog.log";
#      else
    static const char* kDefaultLogFile                         =      "/sdcard/minetang/myeasylog.log";
#      endif  // ELPP_OS_ANDROID
#   elif ELPP_OS_WINDOWS
            ...
            try {
                ...
                ELPP_INTERNAL_ERROR("Unable to get configuration [" << confName << "] for level [" 
                    << LevelHelper::convertToString(level) << "]"
                         << "Please ensure you have properly configured logger.", false);
                return Conf_T();
            }
        }
        return it->second;
...
static void logCrashReason(int sig, bool stackTraceIfAvailable, Level level, const char* logger) {
    ...
    if (stackTraceIfAvailable) {
        ss  << "    ======= Backtrace: ========="  << base::debug::StackTrace();
    }
#else
